...
parser.add_argument("--clear", "-c", dest="ISCLEARED", help="Clear directory before dumping", default=False, action="store_true")
...
#Setup dumps folder
if clear and os.path.isdir("dumps"):
	log("Clearing old dumps...")
	shutil.rmtree("dumps")
username = client.get_entity("me").username
current_time = datetime.now().strftime("%Y.%m.%d %H-%M-%S")
dump_folder = os.path.join("dumps", parse_file_name("{} ({})".format(username, number)), current_time)
...
for dialog in dialogs:
	...
	path = os.path.join(dump_folder, file_name + ".txt")
...
if is_audios:
	...
	for dialog in dialogs_content:
		...
		if (len(audios) > 0):
			#Prepare path to export
			audios_path = os.path.join(dump_folder, "audios", parse_file_name(dialog))
...
if is_photos:
	...
	for dialog in dialogs_content:
		...
		if (len(photos) > 0):
			#Prepare path to export
			photos_path = os.path.join(dump_folder, "photos", parse_file_name(dialog))
